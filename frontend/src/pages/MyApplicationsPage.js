...
function RateProviderModal({ app, onClose, onDone }) {
  ...
  const submit = async () => {
    setSaving(true);
    try {
      await api.post('/ratings', { jobId: app.job._id, rateeId: app.job.provider, value, feedback });
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center p-4">
      <div className="bg-navy-700 rounded-2xl w-full max-w-sm p-5 space-y-4">
        <h3 className="font-bold text-white">{t(lang, 'rateProvider')}</h3>
...
export default function MyApplicationsPage() {
  ...
  return (
    <div className="px-4 py-5 max-w-lg mx-auto space-y-4">
      <h1 className="text-xl font-bold text-white">{t(lang, 'myApplications')}</h1>
        ...
        {TABS.map((tabKey) => {
          ...
          return (
            <button key={tabKey} onClick={() => setTab(tabKey)}
              className={`flex-shrink-0 text-xs font-medium px-4 py-2 rounded-full transition-colors ${
                tab === tabKey ? 'bg-primary-400 text-white' : 'bg-navy-700 text-slate-400 border border-white/10'
              }`}>
              {tabKey === 'all' ? t(lang, 'all') : tStatus(lang, tabKey)}
      ...
      {filtered.length === 0 ? (
        <div className="bg-navy-700 rounded-2xl p-10 text-center shadow-sm">
          ...
          <p className="text-white font-semibold">{t(lang, 'noApplications')}</p>
          <p className="text-slate-400 text-sm mt-1">{t(lang, 'browseAndApply')}</p>
          ...
          {filtered.map((app) => (
            <div key={app._id} className="bg-navy-700 rounded-2xl p-4 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <div className="flex-1 cursor-pointer" onClick={() => navigate(`/jobs/${app.job?._id}`)}>
              ...
              <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
                <span className="flex items-center gap-1 text-xs text-slate-400">
                  <MapPin className="w-3 h-3" /> {app.job?.location?.address || '—'}
                </span>
                <span className="flex items-center gap-1 text-xs text-slate-400">
                  <Clock className="w-3 h-3" /> {tJobType(lang, app.job?.jobType)}
                </span>
                {app.job?.startDate && (
                  <span className="flex items-center gap-1 text-xs text-slate-400">
              ...
              {app.proposedSalary && (
                <div className="flex items-center gap-2 mt-2 bg-orange-500/10 border border-orange-500/20 rounded-lg px-2.5 py-1.5">
                  <span className="text-xs text-orange-400 font-semibold">₹{app.proposedSalary.toLocaleString()}/{tPeriod(lang, app.job?.salary?.period)}</span>
                  <span className="text-[10px] text-slate-500">· ₹{app.job?.salary?.amount?.toLocaleString()}</span>
                </div>
              )}
              ...
              <div className="flex items-center justify-between mt-3">
                <span className="text-primary-400 font-bold text-sm">
                  ...
                  <span className="text-slate-400 font-normal text-xs"> /{tPeriod(lang, app.job?.salary?.period)}</span>
                </span>
                <div className="flex gap-2">
                  {app.status === 'accepted' && (
                    <button onClick={() => openWhatsApp(app.job)}
                      className="flex items-center gap-1 bg-green-500 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-green-600">
                      <MessageCircle className="w-3.5 h-3.5" /> {t(lang, 'chat')}
                    </button>
                  )}
                  {app.status === 'accepted' && app.job?.status === 'completed' && (
                    <button onClick={() => setRatingTarget(app)}
                      ...
                      <Star className="w-3.5 h-3.5" /> {t(lang, 'rateProvider')}
                    </button>
                  )}
                  {app.status === 'pending' && (
      ...
      {ratingTarget && (
        <RateProviderModal app={ratingTarget} onClose={() => setRatingTarget(null)} onDone={() => setRatingTarget(null)} />
      )}
    </div>
  );
}
