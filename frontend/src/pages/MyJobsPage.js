    ...
    <div className="border border-white/10 rounded-xl p-3 bg-navy-800">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-1">
          <div className="relative w-8 h-8 rounded-full bg-primary-400/20 flex items-center justify-center flex-shrink-0">
        ...
        <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize flex-shrink-0 ${APP_STATUS_STYLE[app.status]}`}>
          {app.status}
        </span>
      </div>
      ...
      {app.proposedSalary && (
        <div className="mt-2 flex items-center gap-1.5 bg-orange-500/10 border border-orange-500/20 rounded-lg px-2.5 py-1.5">
          <span className="text-xs text-orange-400 font-semibold">Proposed: ₹{app.proposedSalary.toLocaleString()}</span>
          <span className="text-[10px] text-slate-500">(posted: ₹{jobSalary?.amount?.toLocaleString()})</span>
        </div>
      )}
      {app.coverNote && (
        <p className="mt-2 text-xs text-slate-400 italic">"{app.coverNote}"</p>
      )}
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-end justify-center p-4">
      <div className="bg-navy-700 rounded-2xl w-full max-w-sm p-5 space-y-4">
        <h3 className="font-bold text-white">Rate {app.applicant?.name}</h3>
...
export default function MyJobsPage() {
  ...
  const handleAppStatus = async (appId, status, jobId) => {
    try {
      ...
      setApplicantsMap((m) => ({
        ...m,
        [jobId]: m[jobId].map((a) => a._id === appId ? { ...a, status } : a),
      }));
  ...
  return (
    <div className="px-4 py-5 max-w-lg mx-auto space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-white">My Job Posts</h1>
        <button
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{job.category} · {job.jobType}</p>
                  </div>
                  <span className={`text-xs font-medium px-2.5 py-1 rounded-full capitalize flex-shrink-0 ${STATUS_STYLE[job.status]}`}>
                    {job.status}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <span className="text-xs text-slate-400">{job.filledVacancies}/{job.vacancies} filled</span>
                </div>
                  ...
                  {loadingApps === job._id ? (
                    ...
                    applicantsMap[job._id].map((app) => (
                      <ApplicantCard
                        ...
                        onRate={(app) => setRatingTarget({ app, jobId: job._id })}
                      />
                    ))
                  )}
                </div>
